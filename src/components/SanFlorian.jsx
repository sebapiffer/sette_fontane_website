...
export default function SanFlorian() {
  ...
  // L'indicatore "scorri per approfondire" (fisso al fondo) sfuma allo stesso
  // modo, su ogni viewport: ha fatto il suo lavoro appena l'utente scorre.
  const onOverlayScroll = (e) => {
    if (!zoomed) return;
    const progresso = Math.min(e.currentTarget.scrollTop / 260, 1);
    gsap.set(".sf-scroll-hint", { autoAlpha: 1 - progresso });
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Indicatore di scroll verso la storia del vino: stessa struttura
            dell'indicatore della Hero (testo + linea verticale pulsante), ma
            ancorato FISSO al fondo del viewport, non in-flow: in-flow finiva
            sotto la piega (su desktop il primo blocco è alto min-h-full) e
            nella schermata iniziale non si vedeva mai. Sfuma con lo scroll
            dell'overlay (onOverlayScroll), come la bottiglia su mobile. Il
            wrapper è pointer-events-none per non coprire i contenuti sotto;
            lg:pr-[42vw] lo centra sulla colonna di testo, non sul viewport
            (a destra c'è la bottiglia). */}
        <div className="pointer-events-none fixed inset-x-0 bottom-0 z-[70] bg-gradient-to-t from-[#181816] to-transparent lg:pr-[42vw]">
          <a
            href="#san-florian-storia"
            className="sf-detail sf-scroll-hint pointer-events-auto mx-auto flex w-fit flex-col items-center gap-3 pb-5 pt-2 text-offwhite/60 transition-colors hover:text-offwhite"
          >
            <span className="text-[0.62rem] uppercase tracking-[0.4em]">
              {sanFlorian.storia.hint}
            </span>
            <span
              className="block h-10 w-px animate-pulse bg-current"
              aria-hidden="true"
            />
          </a>
        </div>

        {/* La storia: stesso contenitore e stessa colonna (max-w-xl, bordo
            sinistro) del blocco di degustazione sopra, così i due testi sono
            incolonnati identici — il lg:pr-[42vw] sta DENTRO il max-w-7xl
            come sopra, non fuori, o il centraggio mx-auto slitta a sinistra;
        ...
        >
          <div className="max-w-xl">
            <SplitHeading
              as="h2"
              className="sf-detail font-display text-[clamp(1.8rem,4vw,2.6rem)] leading-[1.1] text-offwhite"
            >
              {sanFlorian.storia.titolo}
            </SplitHeading>
